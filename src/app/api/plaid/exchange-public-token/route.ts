...
import { detectPreviousPlaidItemsForInstitution } from '@/lib/plaid/retirePreviousPlaidItemsForInstitution';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // If this bank was already connected, surface the prior item(s) for the same
        // institution so we can flag possible duplicates — but never delete them here
        // (a user can legitimately hold separate connections at one bank). Detection only.
        let possibleDuplicates: Awaited<
            ReturnType<typeof detectPreviousPlaidItemsForInstitution>
        > = [];
        try {
            possibleDuplicates = await detectPreviousPlaidItemsForInstitution({
                userId: user.id,
                institutionId,
                keepPlaidItemDbId: plaidItem.id,
            });
        } catch (err) {
            console.error('Error detecting previous Plaid items:', err);
        }
        ...
        return NextResponse.json({
            ...
            possible_duplicate_items: possibleDuplicates,
